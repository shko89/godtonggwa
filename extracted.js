        ...
        let myExamData = {};  
        let myExamResults = {}; 
        let myWrongAnswers = []; // [추가] 오답노트 데이터를 저장할 전역 변수
        ...
        window.downloadExamPaper = async (docId, examTitle) => {
            ...
            try {
                ...
                if (!pdfRes.ok) throw new Error(`파일을 찾을 수 없습니다.`);

                const existingPdfBytes = await pdfRes.arrayBuffer();
                const headerString = String.fromCharCode(...new Uint8Array(existingPdfBytes.slice(0, 5)));

                if (headerString !== '%PDF-') throw new Error(`PDF 파일이 아닙니다!`);
                ...
                try {
                    const fontRes = await fetch(`./fonts/watermark.ttf`);
                ...
                let watermarkText = useFallback ? `${currentUser.email}` : `${currentUserData.name} / ${currentUserData.email}`;

                pages.forEach(page => {
                    page.drawText(watermarkText, { x: width / 2 - 100, y: height / 2, size: 24, font: customFont, color: rgb(0.8, 0.2, 0.2), opacity: 0.2, rotate: degrees(45) });
                    page.drawText(`Licensed to: ${currentUser.uid}`, { x: 20, y: 20, size: 9, font: customFont, color: rgb(0.5, 0.5, 0.5) });
                });
            ...
            } catch (e) { alert("❌ 오류 발생:\n" + e.message); console.error(e); } finally { loadingEl.classList.add('hidden'); }
        };
                                ...
                                <table class="w-full text-left border-collapse">
                                    <thead class="bg-gray-50 text-gray-500 text-[10px] uppercase text-center">
                                        <tr>
                                            ...
                                            <th class="px-2 py-2 font-medium w-16">마킹/정답</th>
                    ...
                    snaps.forEach((snap, idx) => {
                        if(snap.exists()) {
                            ...
                            if(qData.stats && qData.stats.totalAttempts > 0) {
                                statsMap[sortedAnswers[idx].qId] = Math.round((qData.stats.correctCount / qData.stats.totalAttempts) * 100);
                            }
                        }
                    });
                } catch(e) { console.error(e); }
            ...
            if (sortedAnswers.length > 0) {
                sortedAnswers.forEach(ans => {
                    ...
                    if (statsMap[ans.qId] !== undefined) {
                        ...
                        rateHtml = `<span class="${color} text-[10px] px-1.5 py-0.5 rounded font-bold ml-1">정답률 ${r}%</span>`;
                    } else {
                    ...
                    tableHtml += `
                        <tr class="hover:bg-gray-50 transition border-b border-gray-100 last:border-0">
                            ...
                            <td class="px-2 py-3 text-gray-600 text-[11px] truncate max-w-[120px]" title="${ans.topic}">${ans.topic} ${rateHtml}</td>
                ...
                tableHtml = '<tr><td colspan="4" class="text-center py-6 text-gray-400 text-xs">기록된 오답이 없습니다.</td></tr>';
            }
        ...
        function renderChartsAndLists(data) {
            ...
            if (wrongAnswersList) {
                    ...
                    try {
                        ...
                        snaps.forEach((snap, idx) => {
                            if(snap.exists()) {
                                const qData = snap.data();
                                if(qData.stats && qData.stats.totalAttempts > 0) {
                                    statsMap[wrongAnswers[idx].qId] = Math.round((qData.stats.correctCount / qData.stats.totalAttempts) * 100);
                                }
                            }
                        });
                    } catch(e) { console.error(e); }
                ...
                if (wrongAnswers.length > 0) {
                    wrongAnswers.sort((a,b) => {
                        ...
                        tableHtml += `
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-2 py-3 text-gray-600 text-[11px] truncate max-w-[120px]" title="${topicText}">${topicText} ${rateHtml}</td>
                                <td class="px-2 py-3 text-center">
                    ...
                    wrongAnswersList.innerHTML = '<tr><td colspan="4" class="text-center py-6 text-gray-400 text-xs">틀린 문항이 없습니다. 완벽합니다! 🎉</td></tr>';
                }
            }
        }
